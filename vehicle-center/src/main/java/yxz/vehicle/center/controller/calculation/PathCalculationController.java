...
public class PathCalculationController {
    ...
    public Object fun1(Integer startPointId, Integer endPointId) {
        ...
        // log.info("开始吧");
        List<RoadInfo> list = roadInfoService.list(); // 获取所有数据(数据量小的时候可以, 如果数据量大那就必须实时获取)
        globalList.clear(); // 清除之前的数据
        getAllPath(list, new ArrayList<>(), startPointId, endPointId); // 执行路径查询程序
        globalList = globalList.stream() // 对所有可能的路径按总长度排序
                .sorted((a, b)-> {
    ...
    private void getAllPath(List<RoadInfo> all,List<RoadInfo> oneRoad, Integer startPointId, Integer endPointId) {
        ...
        for (RoadInfo startRoadInfo : startPaths) {
            ...
            oneNewRoad.add(startRoadInfo); // 下面这两个都需要就单独提取出来
            if (startRoadInfo.getEndPoint().equals(endPointId)) { // 与目标相同, 找到了, 加入总集合中
